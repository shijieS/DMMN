...
class MOT17TrainDataset(Dataset):
    def __init__(self, root=config['dataset_path'],
        ...
        all_list = np.unique([os.path.basename(f)[:8] for f in glob.glob(os.path.join(self.data_folder, "MOT17-*"))])

        if sequence_list is not None:
            sequence_file_list = np.loadtxt(sequence_list, dtype=np.str)
            all_list = [f for f in all_list if f in sequence_file_list]

        all_list = [os.path.join(self.data_folder, f+"-DPM") for f in all_list]
