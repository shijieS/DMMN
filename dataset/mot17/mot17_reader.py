...
class MOT17TestDataset:
    def __init__(self,
        ...
        if sequence_list is None:
            image_folder_list = glob.glob(os.path.join(folder, 'CVPR19-[0-9][0-9]'))
            sequence_list = [os.path.basename(i) for i in image_folder_list]
        else:
            sequence_list = np.loadtxt(sequence_list, dtype=np.str, ndmin=1)
            image_folder_list = [os.path.join(folder, s+'-DPM') for s in sequence_list]
        ...
        image_list = [
            sorted(glob.glob(os.path.join(
                os.path.join(image_folder, 'img1'),
                '[0-9][0-9][0-9][0-9][0-9][0-9].jpg')))
